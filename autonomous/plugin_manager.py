...
PLUGIN_REGISTRY_FALLBACK = "https://raw.githubusercontent.com/AstrBotDevs/AstrBot-Plugins/main/plugins.json"
    ...
    async def _fetch_plugin_registry(self) -> list[dict[str, Any]]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                # 尝试官方 API
                try:
                    async with session.get(PLUGIN_REGISTRY_URL, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                        if resp.status == 200:
                            data = await resp.json()
                            plugins = data if isinstance(data, list) else data.get("plugins", data.get("stars", []))
                except Exception:
                    # 尝试备用源
                    ...
                    async with session.get(fallback_url, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                        if resp.status == 200:
                            data = await resp.json()
