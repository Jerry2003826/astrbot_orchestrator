...
class AstrBotSkillLoader:
    ...
    def _extract_skill_info(self, skill_obj: Any) -> dict[str, Any] | None:
        ...
        try:
            ...
            description = (
                getattr(skill_obj, 'description', None) or
                (skill_obj.get('description') if isinstance(skill_obj, dict) else None) or
                getattr(skill_obj, '__dict__', {}).get('description', '')
            )
            
            path = (
                getattr(skill_obj, 'path', None) or
                (skill_obj.get('path') if isinstance(skill_obj, dict) else None) or
                getattr(skill_obj, '__dict__', {}).get('path', '')
            )
            
            active = (
                getattr(skill_obj, 'active', True) or
                (skill_obj.get('active', True) if isinstance(skill_obj, dict) else True) or
                getattr(skill_obj, '__dict__', {}).get('active', True)
            )
